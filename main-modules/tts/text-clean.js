...
// 没有符号的正文是旁白。朗读规则：
//   1. 优先只提取所有「对话」片段拼接成朗读文本（*内心独白* 和旁白整段丢弃，不读）
//   2. 兜底：如果整条回复里一个「」都没有（模型偶尔没遵守格式），退回旧逻辑——
//      整段照读，只丢掉 *内心独白*，避免因为格式没对上就完全不出声
...
// 兜底路径专用：反复剥最内层括号，处理"（笑（小声））"这类嵌套（旧格式遗留，新 prompt 不会再让模型这么写）
...
function prepareSpeechText(raw) {
  ...
  if (quoted.length) {
    ...
    // 兜底：没有任何「」，说明模型这次没按格式输出，整段按旧逻辑处理，只丢内心独白
    s = s.replace(/[*＊][^*＊]*[*＊]/g, ' ');
    s = s.replace(/[*＊]/g, '');
    s = stripBrackets(s);
  }
